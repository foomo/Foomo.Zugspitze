...
class Upload 
{
	 ...
	 * @return Zugspitze\Services\Upload\Info
	 */
	public function chunkUpload($chunk, $totalLength, $uploadName, $uploadId=null)
	{
		if (empty($uploadId)) {
			$filename = tempnam(\Zugspitze\Upload::getTmpDir(), 'upload_');
			$fp = fopen($filename, 'w');
		} else {
			$filename = \Zugspitze\Upload::getTmpDir() . DIRECTORY_SEPARATOR . basename($uploadId);
		...
		$uploadInfo = new \Zugspitze\Services\Upload\Info(basename($filename));
