...
class Server
{
	...
	public static function serve()
	{
		if (count($_FILES)) {
			...
			foreach ($_FILES as $id => $uploadArray) {
				$uploadInfo = tempnam(\Zugspitze\Upload::getTmpDir(), 'upload-');
				$id = basename($uploadInfo);
				move_uploaded_file($uploadArray['tmp_name'], $uploadInfo);
				$uploadArray['tmp_name'] = $uploadInfo;
				$upload = new \Zugspitze\Services\Upload\Info($id, $uploadArray);
	...
	private static function reflectUploadedFile()
	{
		if (!empty($_GET['id'])) {
			...
			if (!is_null($ref) && $ref instanceof \Zugspitze\Services\Upload\Info) {
	 ...
	 * @return Zugspitze\Services\Upload\File
	 */
	public static function getUpload($uploadId)
	{
		return \Zugspitze\Services\Upload\Info::load($uploadId);
	}
}